...
plot_title = "XLF LSTM Train 03_02, N: 1, TS: 2"

# Plot scatterplot 
plot_query = """
SELECT * 
FROM lstm_madness_train_03_02
ORDER BY index
"""
...
(
  ...
  , SUM(prediction) OVER (ORDER BY dt) AS cum_predict
  , CASE 
      WHEN LAG(prediction, 1) OVER (ORDER BY dt) > 0 THEN -ret * 100 * abs(prediction)
      WHEN LAG(prediction, 1) OVER (ORDER BY dt) < 0 THEN ret * 100 * abs(prediction)
      ELSE 0
      END AS profit
  FROM lstm_madness_train_03_02
...
plot_cum_profit_df = pd.read_sql_query(plot_partial_size_query, con=conn)

ggplot(pd.melt(plot_cum_profit_df, id_vars=['dt']), aes(x='dt', y='value', color='variable')) +\
    geom_line() +\
    ggtitle(plot_title) +\
    labs(y = "PNL - partial bets")